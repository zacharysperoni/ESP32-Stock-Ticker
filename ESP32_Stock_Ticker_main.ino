...
// ===== Finnhub =====
const char* apiKey = "d461299r01qieo4scau0d461299r01qieo4scaug";

// ===== MAX7219 / MD_Parola setup =====
...
// ===== Tickers (no VIX) =====
const char* tickers[] = {"SPY", "QQQ", "BINANCE:BTCUSDT", "BINANCE:ETHUSDT"};
const char* labels[]  = {"S&P", "NQ", "BTC", "ETH"};
const int   numTickers = 4;
  ...
  for (int i = 0; i < numTickers; i++) {
    ...
    if (!isnan(q.price) && !isnan(q.pct)) {
      snprintf(tickerStr, sizeof(tickerStr),
               "%s: %.0f %.1f%% ", labels[i], q.price, q.pct);
    } else {
      snprintf(tickerStr, sizeof(tickerStr), "%s: -- -- ", labels[i]);
    }
...
void loop() {
  ...
  if (open && !displayOn) {
    ...
    P.setIntensity(0);       // optional but fine
